...
def checkNumber(data_source, data_class, attrstr, effi):
    ...
    for value in data_source[attrstr]:
        ...
        if type(v) is np.int64:
            if effi == 10:
                data_class.loc[idx,attrstr] = v#int(math.log10(v+1))
            elif effi == 2:
                data_class.loc[idx,attrstr] = v#int(math.log2(v+1))
            else:
                data_class.loc[idx,attrstr] = v#int(math.log(v+1, effi))
        else:
...
data_trai.head(10)
print(len(data_trai),data_trai.columns)
data_test = DataFrameFilter(sData_test)
data_test.head(10)
print(len(data_test),data_test.columns)
...
for i in range(100):
    ii = i % 20
    jj = int(i / 20)
    clf = GradientBoostingClassifier(n_estimators=100, learning_rate=ii*0.1+0.1 ,max_depth=jj+1, random_state=0).fit(thedatatr, targettr)
    thetar = clf.predict(thedatate)
    accuracy = 0.0
    precision = 0.0
    recall = 0.0
    f1score = 0.0
    ...
    for i in range(len(thetar)):
        if thetar[i]==1:
            ...
            if data_test['bot'][i]==1:
                ...
                tn+=1;
    accuracy += (tp+tn)/len(thetar)
    if maxa<accuracy:
        maxa = accuracy
        print(ii,jj,accuracy)
        print(tp,fp,fn,tn)
print(maxa,ii,jj)
