...
def checkNumber(data_source, data_class, attrstr, effi):
    ...
    for value in data_source[attrstr]:
        if type(value) is int:
            if effi == 10:
                data_class.loc[idx,attrstr] = int(math.log10(data_source[attrstr][idx]+1))
            elif effi == 2:
                data_class.loc[idx,attrstr] = int(math.log2(data_source[attrstr][idx]+1))
            else:
                data_class.loc[idx,attrstr] = int(math.log(data_source[attrstr][idx]+1, effi))
        else:
            data_class.loc[0,attrstr]=0
        idx += 1
...
for k in range(1):
    ...
    result = model.getResult(data_test, ma, 'bot',testindexlist )
    print(len(result));
    headers = ['id','bot']
    rows=[];
